...
class ChildConnectionTableView : BackConnectionTableViewUi() {
	...
	fun initialize() {
		...
		this.fromTC.setCellValueFactory { val conn = it.value; ReadOnlyStringWrapper(conn.foreignKey?.from?.table.toString()) }
		this.fromKeyTC.setCellValueFactory { val conn = it.value; ReadOnlyStringWrapper(conn.foreignKey?.from?.name) }
		this.toTC.setCellValueFactory { val conn = it.value; ReadOnlyStringWrapper(conn.foreignKey?.to?.table.toString()) }
		this.toKeyTC.setCellValueFactory { val conn = it.value; ReadOnlyStringWrapper(conn.foreignKey?.to?.name) }
